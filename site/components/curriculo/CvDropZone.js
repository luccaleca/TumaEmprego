...
export default function CvDropZone({
  ...
  const layout = variant ?? (compact ? "compact" : "default");
  ...
  function handleAbrirPreview(event) {
  ...
  return (
    <div className={isStage ? "w-full" : isCompact ? "w-full p-2" : "w-full p-3 sm:p-4"}>
      <input
        ref={inputRef}
        type="file"
        accept={ACCEPT}
      ...
      <div
        ...
        onDrop={onDrop}
        className={[
          "flex flex-col items-center justify-center rounded-lg border-2 border-dashed text-center transition outline-none",
          isStage ? "w-full" : "w-full",
          "focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2",
          hasPreview
            ? isStage
              ? "border-zinc-200/60 bg-zinc-50/20 px-3 py-3 sm:px-4"
              : "border-zinc-200/80 bg-zinc-50/50 px-3 py-2"
            : isStage
              ? "px-4 py-8"
              : isCompact
                ? "px-3 py-3"
      ...
      >
        {hasPreview ? (
          isStage ? (
            <div className="flex w-full flex-col items-center gap-3 sm:flex-row sm:items-start sm:gap-4 sm:text-left">
              <button
                ...
                className="shrink-0 overflow-hidden rounded-md transition hover:ring-2 hover:ring-emerald-400/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
                aria-label="Abrir currículo principal"
                title="Abrir documento"
              >
                {pdfSrc ? (
                  <CvPdfThumbnail src={pdfSrc} variant="stage" />
                ) : (
                  <CvTextThumbnail text={previewText} variant="stage" />
                )}
              </button>
              <div className="min-w-0 flex-1 text-center sm:text-left">
                {arquivo ? (
                  <>
                    <p className="truncate text-sm font-semibold text-zinc-900">{arquivo.name}</p>
                ...
                <p className="mt-1 text-[11px] text-zinc-400">
                  {uploading ? "Aguarde…" : "Clique na prévia para abrir · arraste para substituir"}
                </p>
                {pdfSrc && arquivo && onAbrirPreview ? (
                  <button
                    type="button"
                    onClick={handleAbrirPreview}
            ...
            <div className="flex w-full items-center gap-3">
              {pdfSrc ? (
                <CvPdfThumbnail src={pdfSrc} compact={isCompact} />
              ) : (
                <CvTextThumbnail text={previewText} compact={isCompact} />
              )}
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="text-sm font-semibold text-zinc-900">
              {uploading ? "Enviando…" : "Currículo principal"}
            </p>
            <p className="mt-1 text-xs text-zinc-500">PDF, Markdown ou texto — até 10 MB</p>
            {!uploading ? (
              <span className="mt-3 inline-flex rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-emerald-700">
                Escolher arquivo
              </span>
            ) : null}
          </div>
      ...
      {message && !isStage ? (
        <p
          className={`${isCompact ? "mt-1.5" : "mt-3"} text-center text-[11px] font-medium ${isError ? "text-red-600" : "text-emerald-700"}`}
          role="status"
        >
          {message}
        </p>
