...
13. **Commits** — só commitar quando o Lucca pedir. Título **curto no imperativo** (como o GitHub usa): `adiciona secao tecnologias`, `remove profile.yml do git` — **não** 1ª pessoa (`eu adicionei…`). Corpo opcional, 1–3 linhas, só se precisar. **Nunca** `Co-authored-by`, `Made-with: Cursor`, nem texto que pareça gerado por IA.
